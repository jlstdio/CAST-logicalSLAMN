...
def _compute_losses_lelan(
    ...
    obs_cond = model("vision_encoder", obs_img=batch_obs_images, input_goal_mask=goal_mask)
...
def train_lelan_lcbc(
    ...
    with tqdm.tqdm(dataloader, desc="Train Batch", leave=False) as tepoch:
        for i, data in enumerate(tepoch):
            ...
            batch_obs_images = [transform(TF.resize(obs, (96, 96), antialias=True)) for obs in obs_images]
            batch_obs_images = torch.cat(batch_obs_images, dim=1).to(device)
            batch_goal_images = transform(TF.resize(goal_image, (96, 96), antialias=True)).to(device)
            ...
            obsgoal_cond = model("vision_encoder", obs_img=batch_obs_images, feat_text=lang_embedding, current_img=batch_obs_current)
            ...
            if i % print_log_freq == 0 and print_log_freq != 0:
                ...
                multi_action_waypts_cos_sim = action_reduce(F.cosine_similarity(
                    torch.flatten(actions_pred[:, :, :2], start_dim=1),
