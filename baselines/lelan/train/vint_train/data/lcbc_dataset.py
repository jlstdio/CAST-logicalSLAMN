...
class LCBCDataset(Dataset):
    def __init__(
        ...
        with open(
            os.path.join(os.path.dirname(__file__), "data_config_lcbc.yaml"), "r"
        ) as f:
            all_data_config = yaml.safe_load(f)
        assert (
            self.dataset_name in all_data_config
        ), f"Dataset {self.dataset_name} not found in data_config_lcbc.yaml"
    ...
    def _get_trajectory(self, trajectory_name):
        if trajectory_name in self.trajectory_cache:
            ...
            if os.path.exists(os.path.join(self.data_folder, trajectory_name, self.traj_file)):
                # print(f"Loading trajectory {trajectory_name} from {self.data_folder}")
                try:
                    with open(os.path.join(self.data_folder, trajectory_name, self.traj_file), "rb") as f:
                        traj_data = pickle.load(f)
                except:
                    traj_data = np.load(os.path.join(self.data_folder, trajectory_name, self.traj_file), allow_pickle=True)
