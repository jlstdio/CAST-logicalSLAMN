...
"""
Prediction functions for PaliVLA.

Modified from PaliGemma in big_vision: https://github.com/google-research/big_vision/blob/main/big_vision/trainers/proj/paligemma/predict_fns.py
"""
...
def _image_avg_repr(
    params, batch: Data, *, model: PaliVLAModel, key: str = "img/pre_logits"
):
...
def _bon_select(state, *, n, eos_token):
    ...
    state = jax.tree.map(lambda x: einops.rearrange(x, "(b n) l -> b n l", n=n), state)
    best_indices = jnp.argmax(scores, -1)  # [b]
    state = jax.tree.map(
        lambda x: jnp.take_along_axis(x, best_indices[:, None, None], axis=1), state
    )
...
def _sample_logits(logits: jnp.ndarray, sampler: str):
    ...
    rng = jax.random.PRNGKey(jax.lax.rng_uniform(0, np.iinfo(np.int32).max, tuple()))
    ...
    sampled_logp = jnp.take_along_axis(
        jax.nn.log_softmax(logits, axis=-1), sampled_tokens[..., None], -1
    )[..., 0]
...
@registry.Registry.register("palivla_sampler.temperature")
def _temperature_sampling(t: float, *, logits: jnp.ndarray, rng: jnp.ndarray):
    return jax.random.categorical(rng, logits / t)


@registry.Registry.register("palivla_sampler.nucleus")
def _nucleus_sampling(
    p: float, t: float = 1.0, *, logits: jnp.ndarray, rng: jnp.ndarray
):
    logits = logits / t
    neg_inf = np.array(-1.0e7)  # Effective negative infinity.
    logits_sorted = jnp.sort(logits, axis=-1, descending=True)
    sorted_cum_probs = jnp.cumsum(jax.nn.softmax(logits_sorted, axis=-1), axis=-1)
...
def _beam_sample_output(state, logits, cache, *, beam_size, max_decode_len, eos_token):
    ...
    cache, logits = jax.tree.map(
        lambda x: einops.rearrange(x, "(b n) ... -> b n ...", b=bs), (cache, logits)
    )
    ...
    scores = jnp.where(topk_tokens != eos_token, scores, jnp.finfo(scores.dtype).min)
    ...
    sampled_tokens = jnp.take_along_axis(topk_tokens, topk_indices, axis=-1)[..., None]
    sampled_logits = jnp.take_along_axis(topk_logits, topk_indices, axis=-1)[..., None]
    ...
    sampled_tokens, cache = jax.tree.map(
        lambda x: einops.rearrange(x, "b n ... -> (b n) ..."), (sampled_tokens, cache)
    )
